...
2) Weekly DAU (7-day rolling)
```sql
SELECT
  day,
  SUM(dau) OVER (ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) / 7 AS rolling_7d_avg
FROM (
...
3) Conversion: discover view -> save
- Approach: for each unique user-day, count users who had a discover_view, and subset who performed save within 24h.

```sql
WITH users_discover AS (
  SELECT DISTINCT COALESCE(user_id, anon_id) AS uid, date_trunc('day', timestamp) AS day
  FROM events
  WHERE event = 'discover_view'
...
SELECT
  d.day,
  COUNT(DISTINCT d.uid) AS discover_users,
  COUNT(DISTINCT s.uid) AS save_users,
  (COUNT(DISTINCT s.uid)::float / NULLIF(COUNT(DISTINCT d.uid),0))::numeric(5,3) AS conversion_rate
FROM users_discover d
LEFT JOIN users_save s USING(uid, day)
GROUP BY d.day
ORDER BY d.day;
```

4) AI conversion rate: ai_upload -> ai_project_generated
```sql
WITH uploads AS (
  SELECT DISTINCT COALESCE(user_id, anon_id) AS uid, timestamp AS upload_ts, properties->>'upload_id' AS upload_id
  FROM events
  WHERE event = 'ai_upload'
    AND timestamp >= now() - interval '30 days'
),
generations AS (
  SELECT properties->>'upload_id' AS upload_id
  FROM events
  WHERE event = 'ai_project_generated'
    AND timestamp >= now() - interval '30 days'
)
SELECT
  COUNT(DISTINCT uploads.upload_id) AS uploads,
  COUNT(DISTINCT generations.upload_id) AS generated,
  (COUNT(DISTINCT generations.upload_id)::float / NULLIF(COUNT(DISTINCT uploads.upload_id),0))::numeric(5,3) AS ai_conversion_rate
FROM uploads
LEFT JOIN generations USING (upload_id);
```
Note: if upload_id not available, match by user + time window (e.g., generation within 10 minutes of upload).

5) Vendor contacts per vendor
```sql
SELECT
  properties->>'vendor_id' AS vendor_id,
  COUNT(*) AS contact_count,
  COUNT(DISTINCT COALESCE(user_id, anon_id)) AS unique_users
FROM events
WHERE event = 'vendor_contact'
  AND timestamp >= now() - interval '30 days'
GROUP BY vendor_id
...
6) Demo signups by campaign
```sql
SELECT
  properties->>'campaign' AS campaign,
  COUNT(*) AS signups
FROM events
WHERE event = 'demo_signup'
  AND timestamp >= now() - interval '90 days'
GROUP BY campaign
ORDER BY signups DESC;
```

7) Retention cohort (example, day-0 -> day-7)
Basic cohort retention via SQL can be heavy; prefer analytics engine. Example approach in Postgres: create first_event_day per user and count active on day N.

## 10. Dashboard suggestions (wireframe)
- Overview (top row)
  - DAU (sparkline 30d)
  - Weekly active users (7d avg)
  - New signups/day
  - AI conversion rate (uploads -> generation)
- Acquisition & Marketing
  - Demo signups by campaign
  - Vendor contacts by vendor & conversion to lead
- Engagement & Retention
  - Saves/day
  - Average session length (if tracked)
  - Game sessions created/joined per day
- Monetization (if applicable)
  - Pricing view -> checkout conversion
  - Revenue by SKU
- Quality & Ops
  - Client errors (top errors)
  - API error rates
- Funnels
  - Discover -> click -> save
  - Upload -> AI generation -> save/share

For each tile, provide link to the underlying SQL (store as saved queries in Metabase/PostHog).

## 11. Implementation plan (MVP)
1. Instrument client events specified above for P0 flows.
2. Build a simple server-side ingest endpoint to validate and write events to Postgres or forward to PostHog.
3. Deploy PostHog OSS (or Matomo) locally for team; or use PostHog Cloud for an immediate demo.
4. Create the dashboards in PostHog/Metabase with the example queries.
5. Run sanity tests: verify event counts vs UI actions for a small sample.
6. Review PII handling & run privacy checklist.

## 12. Privacy checklist (ready-to-run)
- [ ] Strip EXIF/location metadata from uploads unless consent_location true.
- [ ] Do not include email/phone in event properties; store only user_id or hashed identifier.
- [ ] Log consent flags in identity store; include consent_tracking in events.
- [ ] Retain raw events for 90 days.
- [ ] Implement delete-anonymize pipeline for data deletion requests.
- [ ] Limit access to raw event storage and enable query-level ACLs for dashboards.

## 13. Acceptance criteria mapping
- Complete event list covering P0 features: discover_view, item_impression, item_click, save, ai_upload, ai_project_generated, game_session_created/joined, vendor_contact, bulk_request_submitted, xp_earned/xp_spent, demo_signup, auth events, error events. (See section 5)
- Recommended free/open-source tools and deployment notes: PostHog OSS, Matomo, Plausible, or server-side Postgres + Metabase. (See section 2)
- Dashboard wireframes and queries for DAU, save conversion, AI conversion rate, vendor contacts per vendor, demo signups. (See section 9)
- Data retention & PII/consent plan: store coarse geo only, require explicit consent for precise GPS, no raw PII in events, raw event retention 90 days. (See section 8)

## 14. Next steps & ownership
- Owner: assign an analytics owner for instrumentation QA.
- Developer tasks:
  - Add client events where triggered (map UI actions to event names).
  - Add server-side event validation & strip PII logic.
  - Deploy PostHog or set up Postgres+Metabase and load initial dashboards.
- QA:
  - Create a test plan with sample user flows and verify events emitted with expected properties.

---

Appendix: Example minimal ingestion schema (Postgres)
```sql
CREATE TABLE events (
...
CREATE INDEX ON events ((properties->>'vendor_id'));
```
